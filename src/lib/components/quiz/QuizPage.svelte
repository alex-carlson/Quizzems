<script>
	...
	async function saveQuizToCache() {
		...
		try {
			...
			addToast({
				type: 'success',
				message: `Saved ${count} image(s) and quiz data for offline use.`
			});
			...
			saveProgressMessage =
				saveProgressTotal > 0 ? 'Offline cache complete.' : 'Offline save complete.';
	...
	onMount(async () => {
		...
		if (!isOnline) {
			...
			loading = false;
			offlineError =
				'This quiz has not been saved for offline use yet. Open it while online and save it first.';
			return;
		}
		...
		try {
			...
			if (idList.length > 0) {
				const results = await Promise.all(idList.map((id) => quiz.fetchCards(id)));
			...
			{:else}
				...
				{:else}
					<button
						...
						style="width: auto; padding: 0 rem;"
						on:click={() => saveQuizToCache()}>Save for offline</button
					>
				{/if}
					...
					<div class="mt-3">
						<div class="progress" style="height: 0.75rem;">
							<div
								...
								role="progressbar"
								style="width: {saveProgressTotal
									? Math.round((saveProgress / saveProgressTotal) * 100)
									: 100}%"
